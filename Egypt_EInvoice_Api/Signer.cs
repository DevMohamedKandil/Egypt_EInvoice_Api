...
public class TokenSigner
   ...
   // private string TokenPin = "64794808";//  فيرست الرقم السرى للفلاشة
    private string TokenPin = "95644851";//  الرقم السرى مصنع الجوهرى
    private string TokenCertificate = "Egypt Trust Sealing CA";
        ...
        {
           // serializedJson = serializedJson.Replace("0", "0.0");//new
        ...
        else
        {
            ...
            if (request.Type == JTokenType.Property)
            {
                ...
                foreach (var property in request)
                {
                    ...
                    if (property.Type == JTokenType.Boolean || property.Type == JTokenType.Integer || property.Type == JTokenType.Float  )
                    {
                        serialized += "\"" + property.Value<string>() + "\"";
                    }
                    if (property.Type == JTokenType.Date)
                    {
                        serialized += "\"" + DateTime.Parse(property.Value<string>()).ToString("yyyy-MM-dd")+"T23:59:59Z" + "\"";
